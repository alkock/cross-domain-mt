"""
This file consists of the code for the HE to IHC fold experiment.
The GroupKFold consists of k=12 folds, on WSI level (first prefix).
"""
...
NUM_WORKERS = 8
OUTPUT_DIR = Path(__file__).parent.parent / "output" / "heihc_fold"
...
def main():
    ...
    all_ihc_paths = [train_ds.target_dir / p.name for p in train_ds.input_paths] + [
        val_ds.target_dir / p.name for p in val_ds.input_paths
    ]
    # WSI-ID before id.
    groups = [p.stem.split("_id")[0] for p in all_paths]
    ...
    for fold, (train_idx, val_idx) in enumerate(
        ...
        train_ihc_paths = [all_ihc_paths[i] for i in train_idx]
        ...
        # Fold cost calculation: feature-space distance between HE and IHC (train split)
        for model_name, extractor in extractors.items():
            compute_and_save_fold_costs(
                extractor, train_paths, train_ihc_paths, fold_dir, model_name
            )
