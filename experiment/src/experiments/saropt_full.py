...
The experiment is conducted on the full dataset using the created
train/val/test split from sen12_split.ipynb, located in
experiment/preprocessing/sen12_split.ipynb.
"""
...
NUM_WORKERS = 8
OUTPUT_DIR = Path(__file__).parent.parent / "output" / "saropt_full"
...
def main():
    ...
    # Cost calculation: feature-space distance between SAR and OPT (train split)
    train_opt_paths = [train_ds.target_dir / p.name for p in train_ds.input_paths]
    ...
    for model_name, extractor in extractors.items():
        compute_and_save_costs(
            extractor, train_ds.input_paths, train_opt_paths, OUTPUT_DIR, model_name
        )

    # Save explicit filenames for reference
